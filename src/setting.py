...
preprocessing_prompt = """
    <역할>
    해당 스크립트는 요리 레시피를 알려주는 스크립트야. 너는 이 스크립트에서 요리에 관련되지 않은 불필요한 문장들을 제거할 거야.
    다음 순서에 맞게 스크립트를 수정해줘.

    <순서>
    1. 해당 스크립트가 어떤 요리를 목표로 진행하는 지 파악한다.
    2. 스크립트에서 어떤 재료들이 사용되는 지 파악한다.
    3. 이때, 재료들은 요리에 사용되고, 먹을 수 있는 것들이다.
    4. 이제 해당 스크립트에서 요리, 재료, 조리 과정에 불필요한 문장들을 제거한다. 
    5. 전처리 과정을 마친 문장을 출력한다.
"""
